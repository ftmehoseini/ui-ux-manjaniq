...
export interface EventDetail extends EventSummary {
   ...
   * member off to it; when null, registration happens in-product at
 ...
 *
 * `confirmed` is the only status that means a seat is held. The product
 * selects participants, so a backend that reviews requests reports
 * `pending_review` and the success screen says so rather than implying a
 * confirmed seat.
 */
export type RegistrationStatus = "confirmed" | "pending_review" | "waitlisted";

/**
 * What the backend returns after a registration. The success screen renders
 * this and nothing else — every name, count and time on it is server truth,
 * never the form's optimistic echo.
 */
